...
async def сompatibility(ctx, member : discord.Member):
        ...
        embed=discord.Embed(title="Совместимость :",:rat: ,description="**Идёт подсчёт между пользователями**", color=0xcd60aa)
        ...
        embed.add_field(name="Результат:", value=f"Совместимость {ctx.author.mention} и {member.mention},равна {random.choice(list)}  %  ", inline=True)
        await ctx.send(embed=embed) 
